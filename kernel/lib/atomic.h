...
static inline bool set_bit(volatile void* base, u64 offset) {
    u64 mask = ((u64) 1) << (offset % 64);
    return AMO_OP_D(or, mask, ((volatile u64*) base)[offset / 64]) & mask;
}
...
static inline bool change_bit(volatile void* base, u64 offset) {
    u64 mask = ((u64) 1) << (offset % 64);
    return AMO_OP_D(xor, mask, ((volatile u64*) base)[offset / 64]) & mask;
}

// test_bit - Atomically determine whether a bit is set
static inline bool test_bit(volatile void* base, u64 offset) {
    return (((*(volatile u64*) base) >> offset) & 1);
}
